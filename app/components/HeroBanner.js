import Image from "next/image";

export default function HeroBanner() {
  return (
    <section className="hero" id="hero">
      <div className="hero-bg">
        <Image
          src="https://res.cloudinary.com/dltbjoii4/image/upload/v1776185156/chanan/sjjnmkvoj6qnwtjoy430.jpg"
          alt="Tâm An Energy Healing Center"
          fill
          style={{ objectFit: "cover" }}
          priority
          sizes="100vw"
          quality={90}
        />
        <div className="hero-overlay" />
      </div>
      <div className="hero-content">
        <div className="hero-badge">TÂM AN ENERGY HEALING</div>
        <h1 className="hero-title">
          KHOẺ TỪ<br /><span>BÊN TRONG</span>
        </h1>
        <p className="hero-subtitle">
          Trung tâm chăm sóc sức khỏe hàng đầu với công nghệ Hồng ngoại xa (FIR) tiên tiến.
          Mang lại sức khỏe toàn diện cho bạn và gia đình.
        </p>
        <a href="tel:0356308211" className="hero-cta">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z" /></svg>
          Liên hệ tư vấn: 035 630 8211
        </a>
      </div>
    </section>
  );
}
